    ...
    actions: {
        setOrdersList(ordersList: Array<OrderItem>): void {
            this._entities = ordersList.map(item => ({
                ...item,
                dt_created_at: item.created_at ? DateTime.fromFormat(item.created_at, 'YYYY-MM-DD HH:mm:ss') : null,
            }))
        },
        ...
        setOrder(order: Order|null): void {
            ...
            this._entity = {
                ...order,
                dt_created_at: order.created_at ? DateTime.fromFormat(order.created_at, 'YYYY-MM-DD HH:mm:ss') : null,
                dt_updated_at: order.updated_at ? DateTime.fromFormat(order.updated_at, 'YYYY-MM-DD HH:mm:ss') : null,
            }
        }
    }
})
